...
class Countries():
	"""
		Quick and dirty transformer between country name and iso code.

		Falls back on fuzzywuzzy to select most likely country in case
		there isn't an exact match when lookup done by country name.

		Source: http://data.okfn.org/data/core/country-list/r/data.json
	...
	def get_code(self, country, fuzzy_on_error=True):
		try:
			return next(c['Code'] for c in self.data if c['Name']==country)
		except Exception, e:
			if not fuzzy_on_error:
				raise 
			if country in self._cache:
				return self._cache[country]
			else:
				res = process.extractOne(country, [c['Name'] for c in self.data])
				self._cache[country] = next(c['Code'] for c in self.data if c['Name']==res[0])
			return self._cache[country]

	def get_name(self, code):
		return next(c['Name'] for c in self.data if c['Code']==code)